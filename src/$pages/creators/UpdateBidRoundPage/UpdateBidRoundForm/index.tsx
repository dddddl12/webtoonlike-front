...
export function UpdateBidRoundForm({ webtoon }: CreateBidRoundFormProps) {
  ...
  useEffect(() => {
    if (webtoon) {
      if (webtoon.bidRounds && webtoon.bidRounds.length > 0) {
        setOfferRow(
          webtoon.bidRounds[0].contractRange.data.map((cur) =>
            cur.businessField === "webtoon"
              ? { ...cur, contractType: "웹툰 연재권" }
              : { ...cur, contractType: "2차 사업권" }
          ) as OfferRowT[]
        );
        setOriginality(
          webtoon.bidRounds[0].originality as "original" | "notOriginal"
        );
        setIsBrandNew(webtoon.bidRounds[0].isBrandNew ? "true" : "false");
        setNowEpisode(
          webtoon.bidRounds[0].nowEpisode
            ? String(webtoon.bidRounds[0].nowEpisode)
            : ""
        );
        setMonthlyNumEpisode(
          webtoon.bidRounds[0].monthlyNumEpisode
            ? String(webtoon.bidRounds[0].monthlyNumEpisode)
            : ""
        );
        setDoneEpisode(
          webtoon.bidRounds[0].numEpisode
            ? String(webtoon.bidRounds[0].numEpisode)
            : ""
        );
      }
    }
  ...
  async function onSubmit() {
    try {
      ...
      if (offerRow.length > 0) {
        ...
        if (webtoon.bidRounds && webtoon.bidRounds.length > 0) {
          const updated = await BidRoundApi.update(
            webtoon?.bidRounds[0].id,
            insertForm
          );
          await BidRoundApi.get(updated.id, { $webtoon: true });
      ...
      console.warn(e);
      enqueueSnackbar("bid round submit error", { variant: "error" });
    }
  }
